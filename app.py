...
def create_app(config={}):
    ...
    def purchasePlaces():
        if 'username' in session:
            ...
            if placesRequired <= int(club['points']) and placesRequired >= 0 and placesRequired <= int(competition['numberOfPlaces']):

                # ISSUE4 : BUG: Clubs should not be able to use more than their points allowed
                if int(club[str(competition['name'])]) + placesRequired > MAX_PURCHASE:
